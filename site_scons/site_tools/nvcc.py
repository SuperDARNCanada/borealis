...
def add_common_nvcc_variables(env):
    ...
    #MODIFIED TO REMOVE '#' IF PATH IS RELATIVE TO BUILD ROOT
    env['_NVCC_CPPPATH'] = '${_concat("-I", [path.replace("#","") for path in CPPPATH], "", __env__)}'
...
def generate(env):
    ...
    env['STATICNVCCCMD'] = '$NVCC -o $TARGET -c $NVCCFLAGS $STATICNVCCFLAGS $_NVCC_CXXFLAGS $_NVCC_COMMON_CMD $SOURCES'
    env['SHAREDNVCCCMD'] = '$NVCC -o $TARGET -c $NVCCFLAGS $SHAREDNVCCFLAGS -shared $_NVCC_CXXFLAGS $_NVCC_COMMON_CMD $SOURCES'
