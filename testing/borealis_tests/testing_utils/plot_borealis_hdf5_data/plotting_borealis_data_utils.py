...
def plot_output_samples_iq_data(record_dict, record_info_string, sequence=0, real_only=True, antenna_indices=None):
    ...
    for index in indices:
        ...
        if antenna < record_dict['main_antenna_count']:
            ax1.plot(np.arange(record_dict['num_samps']), record_dict['data'][index,sequence,:].real, label='Real {}'.format(antenna))
            if not real_only:
                ax1.plot(np.arange(record_dict['num_samps']), record_dict['data'][index,sequence,:].imag, label="Imag {}".format(antenna))
            ax1.legend()
        else:
            ax2.plot(np.arange(record_dict['num_samps']), record_dict['data'][index,sequence,:].real, label='Real {}'.format(antenna))
            if not real_only:
                ax2.plot(np.arange(record_dict['num_samps']), record_dict['data'][index,sequence,:].imag, label="Imag {}".format(antenna))
            ax2.legend()                       
    plt.show()
...
def fft_and_plot_bfiq_data(record_dict, record_info_string, beam=0, sequence=0, plot_width=None):
    """
    ...
    for index in range(0, record_dict['data'].shape[0]):
        antenna_array = record_dict['antenna_arrays_order'][index]
        if not plot_width:
            fft_samps, xf = fft_to_plot(record_dict['data'][index,sequence,beam,:], record_dict['rx_sample_rate'])
        else:
            fft_samps, xf = fft_to_plot(record_dict['data'][index,sequence,beam,:], record_dict['rx_sample_rate'], plot_width=plot_width)
        len_samples = len(record_dict['data'][index,sequence,beam,:])
        if antenna_array == 'main':
            ax1.set_title('FFT Main Array {} sequence {}'.format(record_info_string, sequence))
            ax1.plot(xf, 1.0/len(fft_samps) * np.abs(fft_samps), label='{}'.format(antenna_array))
        else:
            ax2.set_title('FFT Intf Array {} sequence {}'.format(record_info_string, sequence))
            ax2.plot(xf, 1.0/len(fft_samps) * np.abs(fft_samps), label='{}'.format(antenna_array))      
    ax2.set_xlabel('Hz')             
...
def fft_and_plot_rawrf_data(record_dict, record_info_string, sequence=0, real_only=True, start_sample=18000, end_sample=20000, antenna_indices=None, plot_width=None, center=0):
    """
    :param plot_width: frequency bandwidth to plot fft (for higher resolution) 
    """
...
def fft_and_plot_txdata(record_dict, record_info_string, sequence=0, real_only=True, antenna_indices=None, plot_width=None):
    ...
    for antenna in indices:
        ...
        #    continue
        if not plot_width:
            fft_samps, xf = fft_to_plot(record_dict['data'][sequence,antenna,:], record_dict['tx_rate'])
        else:
            fft_samps, xf = fft_to_plot(record_dict['data'][sequence,antenna,:], record_dict['tx_rate'], plot_width = plot_width)
