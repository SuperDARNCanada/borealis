...
def plot_uhd_txrx_loopback_data(record_array, record_filetype):
    ...
    for antenna in range(record_array.shape[0]):
        ...
        #    continue
        ax1.plot(np.arange(record_array.shape[1]), record_array[antenna,:].real, label='Real {}'.format(antenna))
        ax2.plot(np.arange(record_array.shape[1]), record_array[antenna,:].imag, label="Imag {}".format(antenna))
    ax1.legend()               
    plt.show()
