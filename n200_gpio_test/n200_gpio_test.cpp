...
static const std::string  GPIO_DEFAULT_GPIO_BANK                  = "RXA";  // RXA is the bank on RX daughterboard
static const size_t       GPIO_DEFAULT_NUM_BITS                   = 16;     // There are 16 gpio on the LFRX and LFTX
...
static const float        SEQUENCE_DEFAULT_FIRST_RANGE_IN_KM      = 180;    // Distance to the first range
static const uint32_t     SEQUENCE_DEFAULT_SCOPE_SYNC_DELAY_IN_US = (SEQUENCE_DEFAULT_NUM_RANGES*SEQUENCE_DEFAULT_RSEP_IN_KM + SEQUENCE_DEFAULT_FIRST_RANGE_IN_KM)*1000.0*1000000*2.0/speed_of_light;  // How long after last pulse do we keep scope sync high?
static const uint32_t     SEQUENCE_DEFAULT_SCOPE_SYNC_PRE_DELAY_IN_US  = 50;     // How long before TR to set atten signal high
static const uint32_t     SEQUENCE_DEFAULT_ATTEN_PRE_DELAY_IN_US  = 10;     // How long before TR to set atten signal high
static const uint32_t     SEQUENCE_DEFAULT_TR_PRE_DELAY_IN_US     = 5;      // How long before pulse transmission to set TR signal high
static const uint32_t     SEQUENCE_DEFAULT_ATTEN_POST_DELAY_IN_US = 10;     // How long after TR signal goes low to keep atten signal high for
static const uint32_t     SEQUENCE_DEFAULT_TR_POST_DELAY_IN_US    = 5;      // How long after pulse transmission to keep TR signal high
static const size_t       SEQUENCE_DEFAULT_NUM_PULSES             = 8;      // How many pulses in a pulse sequence?
...
void output_reg_values(const std::string bank, const uhd::usrp::multi_usrp::sptr &usrp, const size_t num_bits) {
  ...
  BOOST_FOREACH(std::string &attr, attrs) {
    std::cout << (boost::format("%10s:%s") % attr % to_bit_string(boost::uint32_t(usrp->get_gpio_attr(bank, attr)), num_bits)) << std::endl;
  }
}
...
void print_gpio_banks(const uhd::usrp::multi_usrp::sptr &usrp) {
  ...
  for(std::vector<std::string>::const_iterator i = all_banks.begin(); i != all_banks.end(); i++) {
    std::cout << *i << " ";
  }
  std::cout << std::endl;
}
...
int UHD_SAFE_MAIN(int argc, char *argv[]) {
  ...
  config.add_options()
    ("help", "help message")
    ("multi_usrp_args", po::value<std::string>(&multi_usrp_args)->default_value(DEVICE_DEFAULT_MULTI_USRP_ARGS)->composing(), "multi uhd device address args")
//    ("cpu_format", po::value<std::string>(&cpu_format)->default_value(GPIO_DEFAULT_CPU_FORMAT), "CPU data format")
//    ("otw_format", po::value<std::string>(&otw_format)->default_value(GPIO_DEFAULT_OTW_FORMAT), "OTW data format")
    ("gpio_bank", po::value<std::string>(&gpio_bank)->default_value(GPIO_DEFAULT_GPIO_BANK), "GPIO bank string")
    ("gpio_ddr", po::value<std::string>(&gpio_ddr)->default_value(GPIO_DEFAULT_GPIO_DDR), "GPIO DDR value string")
    ("gpio_out", po::value<std::string>(&gpio_out)->default_value(GPIO_DEFAULT_GPIO_OUT), "GPIO OUT value string")
    ("gpio_ctrl", po::value<std::string>(&gpio_ctrl)->default_value(GPIO_DEFAULT_GPIO_CTRL), "GPIO CTRL value string")
    ("num_bits", po::value<size_t>(&num_bits)->default_value(GPIO_DEFAULT_NUM_BITS), "Number of bits in GPIO bank")
    ("scope_sync", "Generate scope sync signal")
    ("pulse_sequence", "Generate pulse sequence")
    ("num_pulses", po::value<size_t>(&num_pulses)->default_value(SEQUENCE_DEFAULT_NUM_PULSES), "Number of pulses in the sequence to send")
    ("print_diagnostic_info,d", "Print out various information about the device and exit")
    ("print_gpio_values,g", "Print out the gpio registers periodically (period defined by update_period)")
    ("update_period", po::value<uint32_t>(&update_period_in_s)->default_value(GPIO_DEFAULT_UPDATE_PERIOD_IN_S), "If printing gpio values, how often to print them (s)")
    ("tau", po::value<uint32_t>(&tau_in_us)->default_value(SEQUENCE_DEFAULT_TAU_IN_US), "Tau value for pulse sequence (us)")
    ("pulse_length", po::value<uint32_t>(&pulse_length_in_us)->default_value(SEQUENCE_DEFAULT_PULSE_LENGTH_IN_US), "Pulse length (us)")
    ("scope_sync_delay", po::value<uint32_t>(&scope_sync_delay_in_us)->default_value(SEQUENCE_DEFAULT_SCOPE_SYNC_DELAY_IN_US), "Scope sync delay at end of pulse sequence (us)")
    ("scope_sync_pre_delay", po::value<uint32_t>(&scope_sync_pre_delay_in_us)->default_value(SEQUENCE_DEFAULT_SCOPE_SYNC_PRE_DELAY_IN_US), "Scope sync signal pre-delay before first ATTEN signal (us)")
    ("atten_pre_delay", po::value<uint32_t>(&atten_pre_delay_in_us)->default_value(SEQUENCE_DEFAULT_ATTEN_PRE_DELAY_IN_US), "Attenuator signal pre-delay before TR signal (us)")
    ("tr_pre_delay", po::value<uint32_t>(&tr_pre_delay_in_us)->default_value(SEQUENCE_DEFAULT_TR_PRE_DELAY_IN_US), "TR signal pre delay before pulse output (us)")
    ("atten_post_delay", po::value<uint32_t>(&atten_post_delay_in_us)->default_value(SEQUENCE_DEFAULT_ATTEN_POST_DELAY_IN_US), "Attenuator signal post-delay after TR signal (us)")
    ("tr_post_delay", po::value<uint32_t>(&tr_post_delay_in_us)->default_value(SEQUENCE_DEFAULT_TR_POST_DELAY_IN_US), "TR signal post delay after pulse output (us)")
    ("fib", po::value<uint32_t>(&fib_sequence_number)->default_value(38), "TESTING: fibonacci seq number to calculate up to in pulse train loop")
    ("delay", po::value<uint32_t>(&start_delay_us)->default_value(200), "TESTING: Start delay in us after scope sync to start first pulse's ATTEN signal")
  ;
  ...
  if(vm.count("print_diagnostic_info")) {
    ...
    std::cout << "Number of motherboards: " << usrp->get_num_mboards() << std::endl;
    std::cout << "Low level registers: " << std::endl;
    std::vector<std::string> low_level_registers = usrp->enumerate_registers();
    for(std::vector<std::string>::const_iterator i = low_level_registers.begin(); i != low_level_registers.end(); i++) {
      std::cout << *i << std::endl;
    }
    return 0;
  } 
  ...
  for(std::vector<std::size_t>::const_iterator i = pulses.begin(); i != pulses.end(); i++) {
    std::cout << *i << " ";
  }
  std::cout << std::endl;
  ...
  if (vm.count("pulse_sequence")) {
    ...
    while (not stop_signal_called) {
      ...
      std::chrono::steady_clock::time_point set_command_time_end = std::chrono::steady_clock::now();
      ...
      for (pulse_number = 0; pulse_number < num_pulses; pulse_number++) {
        ...
        if(pulse_number == num_pulses-1) {
          ...
          // Delay is from start of one pulse to start of next pulse, so perform proper calculation here
          proper_interpulse_delay_in_us = tau_in_us*(pulses[pulse_number+1]-pulses[pulse_number]);
       ...
       //   std::cout << "Pulse sequence #: " << pulses[pulse_number] << " next: " << pulses[pulse_number+1] << " Proper interpulse delay (us): " << proper_interpulse_delay_in_us << std::endl;
          accum_time += (double)proper_interpulse_delay_in_us/1000000.0;
          usrp->set_command_time(uhd::time_spec_t(accum_time)); // delay to next pulse
        }
      }

      accum_time += scope_sync_delay_in_us/1000000.0;
      usrp->set_command_time(uhd::time_spec_t(accum_time)); // delay to scope sync low
...
//      sleep(accum_time+0.01); 
      ...
      std::cout << "Set time now: " << std::chrono::duration_cast<std::chrono::microseconds>(set_time_end - pulse_seq_begin).count()
                          << "us" << " Set command time: " << std::chrono::duration_cast<std::chrono::microseconds>(set_command_time_end - set_time_end).count()
                          << "us" << " GPIO attribute time: " << std::chrono::duration_cast<std::chrono::microseconds>(set_gpio_end - set_command_time_end).count()
                          << "us" << std::endl;
    }
  } else {
    // Set the GPIO to the values given in command line options
    std::cout << "Setting GPIO to given values and waiting for CTRL-C to exit..." << std::endl;
    // Note that strtoul has undefined behaviour if the string passed to it is invalid
