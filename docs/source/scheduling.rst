...
The first script in the chain is called ```local_scd_server.py``. This script should run continually,
...
The schedule files need to be synced to the radar sites. The schedule files that the ``local_scd_server`` script modifies
