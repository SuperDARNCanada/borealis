...
Python virtual environment. Then, install the pre-commit hooks with ``pre-commit install``.
Now, every time you commit, ``pre-commit`` will run the hooks for this project and only proceed with the commit if all
hooks pass. Note that some hooks modify the formatting of files, so the hook will fail if it executes any reformatting.
Inspect the changes before attempting to commit again; in most instances, the hooks should pass on the second attempt
without you needing to modify any files. You can run the hooks at any time with ``pre-commit run --all-files``.
