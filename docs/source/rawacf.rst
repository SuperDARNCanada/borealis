...
| blanked_samples | uint32 | number of blanked samples | Samples that have been blanked because they occurred during transmission times, given by sample number (index into decimated data). Can differ from the pulses array due to multiple slices in a single sequence. |
...
| correlation_descriptors | unicode | 4 | Denotes what each correlation dimension (in main_acfs, intf_acfs, xcfs) represents ('num_records' 'max_num_beams' 'num_ranges' 'num_lags') | 
...
| first_range_rtt | float32 | non-array | Round trip time of flight to first range in microseconds. |
...
| intf_acfs | complex64 | num_records x max_num_beams x num_ranges x num_lags | Interferometer array correlations. Note that records that do not have num_beams = max_num_beams will have padded zeros. The num_beams array should be used to determine the correct number of beams to read for the record. |
| intf_antenna_count | uint32 | non-array | Number of interferometer array antennas |
| lags | uint32 | number of lags x 2 | The lags created from two pulses in the pulses array. Dimensions are number of lags x 2. Values have to be from pulses array. The lag number is lag[1] - lag[0] for each lag pair. |
| main_acfs | complex64 | num_records x max_num_beams x num_ranges x num_lags | Main array correlations. Note that records that do not have num_beams = max_num_beams will have padded zeros. The num_beams array should be used to determine the correct number of beams to read for the record. |
...
| num_sequences | int64 | num_records | Number of sampling periods (equivalent to number sequences transmitted) in the integration time for each record. Allows the user to correctly read the arrays with a number of sequences dimension up to the correct number and remove the padded zeros in the data array. |
...
| range_sep | float32 | non-array | Range gate separation (conversion from time (1/rx_sample_rate) to equivalent distance between samples), in km. |
...
| xcfs | complex64 | num_records x max_num_beams x num_ranges x num_lags | Cross correlations of interferometer to main array. Note that records that do not have num_beams = max_num_beams will have padded zeros. The num_beams array should be used to determine the correct number of beams to read for the record. |
...
| blanked_samples | [uint32, ] | Samples that have been blanked because they occurred during transmission times, given by sample number (index into decimated data). Can differ from the pulses array due to multiple slices in a single sequence. |
...
| correlation_descriptors | [unicode, ] | Denotes what each correlation dimension (in main_acfs, intf_acfs, xcfs) represents ('num_beams' 'num_ranges' 'num_lags') | 
| correlation_dimensions | [uint32, ] | The dimensions in which to reshape the acf or xcf datasets. |
...
| first_range_rtt | float32 | Round trip time of flight to first range in microseconds. |
...
| intf_acfs | [complex64, ] | Main array correlations. |
...
| xcfs | complex64 | Cross correlations of interferometer to main array. |
