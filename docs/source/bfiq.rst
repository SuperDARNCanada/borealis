...
| | max_num_beams (in        | | which has num_beams less than the         |
| | any record)]             | | max_num_beams. The num_beams field should | 
...
| | *uint32*                 |                | max_num_beams (in    | | in this record. Note that this is padded  |
| | [num_records x           |                | any record)          | | with zeroes for any record which has      |
| | max_num_beams (in        |                |                      | | num_beams less than the max_num_beams.    |
| | any record)              |                |                      | | The num_beams field should be used to     |
| |                          |                |                      | | read the correct number of beams for each |
| |                          |                |                      | | record.                                   |
+------------------------    +----------------+----------------------+---------------------------------------------+
| | **blanked_samples**      | uint32         | number of blanked    | | Samples that should be blanked because    |
| | *uint32*                 |                | samples              | | they occurred during transmission times,  |
...
| | *complex64*              |                | num_antenna_arrays x | | sample rate. Note that records that do not|
| | [num_records x           |                | max_num_sequences x  | | have num_sequences = max_num_sequences or |
| | num_antenna_arrays x     |                | max_num_beams x      | | num_beams = max_num_beams will have       |
| | max_num_sequences x      |                | num_samps            | | padded zeros. The num_sequences and       |
...
| |                          |                |                      | | ‘num_samps’                               |
+------------------------    +----------------+----------------------+---------------------------------------------+
| | **data_normalization_factor**| float32        | non-array            | | Scale of all the filters used,            |
