...
filter sequence. For an input signal of length :math:`\color{Gray}N` and filter of length :math:`\color{Gray}M`, the output will be of length
:math:`\color{Gray}N+M-1`, as for the first and last few output samples the filter is "hanging off" the input sequence and only a
partial overlap of the sequences contributes to the output. These "hanging off" output samples are discarded by Borealis,
as they have an artifically small magnitude due to fewer samples contributing to them. As a result, only :math:`\color{Gray}N-M+1`
...
(:math:`\color{Gray}num\_channels`, 15000) for each frequency. After the second stage, the number of samples is
...
to correct. If the oscillator has phase :math:`\color{Gray}{\phi}_0 = 0` when it mixes with the zeroth sample,
then it will have phase :math:`\color{Gray}\phi_1 = 2\pi\frac{f_o}{F_s}` when it mixes with the first sample,
:math:`\color{Gray}\phi_2 = 2\pi\frac{f_o}{F_s}2` when it mixes with the second sample, and so on. The general
formula is :math:`\color{Gray}\phi_k = 2\pi\frac{f_o}{F_s}k`, where :math:`\color{Gray}f_o` is the oscillator frequency,
:math:`\color{Gray}F_s` is the data sampling rate, and :math:`\color{Gray}k` is the index of the newest sample. Borealis
applies this correction after applying the filter and downsampling, to reduce the number of
mathematical operations. So, for a downsampling rate of :math:`\color{Gray}R`, the phase correction for sample
:math:`\color{Gray}k` after downsampling is :math:`\color{Gray}\phi_k = 2\pi\frac{f_o}{F_s}Rk`.
...
incoming plane wavefront, and the beam direction off of boresight shown by :math:`\color{Gray}\theta`. For an incoming wave,
we can see that it will hit the leftmost antenna (antenna 0) first, then antenna 1, antenna 2, and
so forth, reaching antenna 15 last. Each antenna :math:`\color{Gray}n` is going to measure a different phase of
the wave, determined by its distance from the reference wavefront :math:`\color{Gray}d_n` as shown in the figure.
The required phase shift can be calculated from the geometry of the diagram as

.. math:: \color{Gray}\phi_n = \frac{d_n}{\lambda} 2\pi

The filtered samples for a given antenna are multiplied by :math:`\color{Gray}\phi_n` to correct their phase,
...
:math:`\color{Gray}d_n` for antennas 0 through 7 will be negative, since the wavefront will have passed them already.
The distance from the reference wavefront for an antenna and beam angle :math:`\color{Gray}\theta` is then

.. math:: \color{Gray}d_n = (n-\frac{N}{2})L\sin(\theta)

where :math:`\color{Gray}n` is the antenna index, :math:`\color{Gray}N` is the total number of antennas in the array, and
:math:`\color{Gray}L` is the uniform antenna spacing. Plugging this result into the previous formula yields ais
final formula of

.. math:: \color{Gray}\phi_n = \left(\frac{L}{\lambda}(n-\frac{N}{2})\sin(\theta)\right) 2\pi
