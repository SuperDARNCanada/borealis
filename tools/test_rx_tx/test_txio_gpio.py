...
class gpio_testing(object):
    ...
    def __init__(self, address, bank):
        ...
        self.input_pins = {}
        if pinout == 'user':
            self.signal_pins["RXO"] = int(input("Specify pin connected to the RXO signal "))
            self.signal_pins["TXO"] = int(input("Specify pin connected to the TXO signal "))
            self.signal_pins["TR"] = int(input("Specify pin connected to the T/R signal "))
            self.signal_pins["IDLE"] = int(input("Specify pin connected to the IDLE signal "))
            self.signal_pins["TM"] = int(input("Specify pin connected to the TEST_MODE signal "))
            self.input_pins["LP"] = int(input("Specify pin connected to the LOW_PWR input "))
            self.input_pins["AGC"] = int(input("Specify pin connected to the AGC_STATUS input "))

            print("Would you like to set up alternate pins for these signals?")
            alternate = input("Enter Y for yes, or enter for no ")
            if alternate == 'Y':
                self.signal_pins["RXO_alt"] = int(input("Specify alternate pin connected to the RXO signal "))
                self.signal_pins["TXO_alt"] = int(input("Specify alternate pin connected to the TXO signal "))
                self.signal_pins["TR_alt"] = int(input("Specify alternate pin connected to the T/R signal "))
                self.signal_pins["IDLE_alt"] = int(input("Specify alternate pin connected to the IDLE signal "))
                self.signal_pins["TM_alt"] = int(input("Specify alternate pin connected to the TEST_MODE signal "))
                self.input_pins["LP_alt"] = int(input("Specify alternate pin connected to the LOW_PWR input "))
                self.input_pins["AGC_alt"] = int(input("Specify alternate pin connected to the AGC_STATUS input "))
        # default pins
        else:
            self.signal_pins = {"RXO": 1,# "RXO_alt": 2,
                    "TXO": 3, #"TXO_alt": 4,
                    "TR":  5, #"TR_alt":  6,
                    "IDLE":7, #"IDLE_alt":8,
                    "TM":  13,"TM_alt":  14}

            self.input_pins = {"LP":  9, #"LP_alt":  10,
                    "AGC": 11}#,"AGC_alt": 12}
        ...
        if len(result) > 0:
            print("Warning: You've selected multiple signals to be routed to one pin. Please double check your entries! Exiting")
            sys.exit(1)
    ...
    def run_single_signals_test(self):
        ...
        for signal, pin, in self.signal_pins.items():
    ...
    def run_differential_signal_test(self):
        ...
        for pin in [self.signal_pins["TR"], self.signal_pins["TM"]]:
            ...
            self.set_all_low()
            print("Initial Status:", hex(self._usrp.get_gpio_attr(self._bank, "READBACK")))
            set_lp_agc_inputs()
            print("Status after setting inputs:", hex(self._usrp.get_gpio_attr(self._bank, "READBACK")))
            time.sleep(1)
            mask = self.get_pin_mask(pin)
            try:
                # run current test
