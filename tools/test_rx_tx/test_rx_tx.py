...
def make_ramped_pulse(tx_rate: float):
	...
	tr_start_pad = 60
	tr_end_pad = 60
	num_samps_per_antenna = int(np.ceil(pulse_len * (tx_rate + tr_start_pad + tr_end_pad)))
	tx_freqs = [1E6]
...
def recv(usrp_d, rx_chans):
	...
	ringbuffer_size = (sys.getsizeof(500.0E6)/np.dtype(np.complex128).itemsize/usrp_buffer_size) * \
						usrp_buffer_size
