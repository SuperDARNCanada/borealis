#!/usr/bin/env python3
# coding: utf-8
"""@package xnec2c_sd_generator

Designed to generate wire structures/loads/etc for using with the NEC program Xnec2c. By using the output of this script as
the input into the xnec2c software, a radiation pattern can be created for SuperDARN antennas. When given as an input to the
SuperDARNCanada repo genetic-array, the widebeam transmitter phases can be found (needed to run the widebeam experiment).
This script is an update of nec_sd_generator.py, which no longer requires a windows emulator (such as wine) to run 4nec2 or eznec.
Xnec2c runs with OpenSUSE Leap 16.0 and full installation instructions are found on the git repo linked below.

The Xnec2c documentation was referenced heavily (https://www.xnec2c.org/)
Also see: https://github.com/KJ7LNW/xnec2c.git for the latest version of Xnec2c (v4.4.18 released Dec 18, 2025)

Authors of nec_sd_generator.py: Kevin Krieger, Adam lozinsky, Remington Rohel
Authors of xnec2c_sd_generator.py: Riley Sylvestre
...
class Tower(object):
    ...
    def __init__(self, start_height_m=15.0, balun_offset=0.075, side_length=0.30, awg=13,
                 global_x=0.0, global_y=0.0, global_z=0.0, guylines=False, without_fence=False, tower_height=1):
        """
        ...
        :param without_fence: Flag to remove reflector fence. Default False
        :param tower_height: Value to indicate 50ft (1) or 30ft (0) tower. Default 1
        ...
        if guylines:
            ...
            # Guy lines at 30ft on 50ft towers
            if (tower_height == 1):
                # Pointing down boresight
...
class TransmissionLine(object):
    ...
    def __init__(self, tag1, segment1, tag2, segment2, crossed, impedance=50.0, length=0.0):
        ...
        if crossed:
            impedance *= -1
        # NOTE: .format() returns a new string, it does not modify in place. The
        # previous version of this code called .format() but discarded the result,
        # which left literal '{}' placeholders in the output instead of the actual
        # values -- this produced an unparseable TL card. The result must be
        # assigned back to self.transmissionline.
        self.transmissionline = "TL {} {} {} {} {} {} 0.0 0.0 0.0 0.0".format(
            tag1, segment1, tag2, segment2, impedance, length)
...
class Wire(object):
    ...
    def rotate(self, alpha, beta, gamma):
        """
        Rotate the wire structure about a local reference point.

        Rotation is performed first about x, then y, then z.

        :param alpha: Rotation about x axis in degrees
        :param beta: Rotation about y axis in degrees
        :param gamma: Rotation about z axis in degrees
        :param origin: (x, y, z) point about which to rotate
        """

        origin = (self.wire_structure['x1'], self.wire_structure['y1'], self.wire_structure['z1'])

        self.rotate_x_axis(alpha, origin)
        self.rotate_y_axis(beta, origin)
        self.rotate_z_axis(gamma, origin)

        # Check that no z coordinate is negative
        # xnec2c cannot handle negative z coords
        if (self.wire_structure['z1'] < 0):
            print("WARNING: Wire {} has a negative z1 coordinate".format(wire_number))
            self.wire_structure['z1'] = 0.1
        if (self.wire_structure['z2'] < 0):
            print("WARNING: Wire {} has a negative z2 coordinate".format(wire_number))
            self.wire_structure['z2'] = 0.1

        return self

    def rotate_x_axis(self, alpha, origin):
        """
        Rotate the wire structure about the x axis through origin (Endpoint 1).

        :param alpha: Rotation angle in degrees
        :param origin: (x, y, z) rotation reference point
        """
        ...
        ox, oy, oz = origin # origin is Endpoint 1

        # Endpoint 2 relative to origin
        x2 = self.wire_structure['x2'] - ox
        y2 = self.wire_structure['y2'] - oy
        z2 = self.wire_structure['z2'] - oz

        # Rotate
        new_y2 = y2 * cos_theta - z2 * sin_theta
        new_z2 = y2 * sin_theta + z2 * cos_theta

        # Translate back
        self.wire_structure['x2'] = x2 + ox
        self.wire_structure['y2'] = new_y2 + oy
        self.wire_structure['z2'] = new_z2 + oz
        return self

    def rotate_y_axis(self, beta, origin):
        """
        Rotate the wire structure about the y axis through origin (Endpoint 1).

        :param beta: Rotation angle in degrees
        :param origin: (x, y, z) rotation reference point
        """
        ...
        # Rotate
        new_x2 = x2 * cos_theta + z2 * sin_theta
        new_z2 = -x2 * sin_theta + z2 * cos_theta

        # Translate back
        self.wire_structure['x2'] = new_x2 + ox
        self.wire_structure['y2'] = y2 + oy
        self.wire_structure['z2'] = new_z2 + oz
        return self

    def rotate_z_axis(self, gamma, origin):
        """
        Rotate the wire structure about the z axis through origin (Endpoint 1).

        :param gamma: Rotation angle in degrees
        :param origin: (x, y, z) rotation reference point
        """
...
class TTFD(object):
    ...
    def __init__(self, wire_gauge=13, height=10.0, termination=100.0,
                 ...
                 global_x=0.0, global_y=0.0, global_z=0.0,
                 current_real=1.0, current_imag=0.0, guylines=False):
        """
...
def create_main_array(num_antennas, antenna_spacing_m,
                      ...
                      log_periodics=False, towers=False, guylines=False, without_fence=False):
    """
    ...
    for antenna in range(0, num_antennas):
        ...
        if log_periodics:
            ...
            antenna_objects.append(TTFD(global_x=x_position,
                                        current_real=real_cur, current_imag=imag_cur, guylines=guylines))
            if towers:
                tower_objects.append(Tower(global_x=x_position, guylines=guylines, without_fence=without_fence, tower_height=1))
    ...
    if towers:
        ...
        for tower in range(0, num_antennas-1):
            ...
            tower_objects.append(Tower(global_x=x_position, start_height_m=12.0, guylines=guylines, without_fence=without_fence, tower_height=0))
    ...
    for antenna in range(0, num_antennas):
        ...
        if log_periodics:
            ...
            antenna_objects.append(TTFD(global_x=global_x_position, global_y=int_y_spacing_m,
                                        ...
                                        current_real=real_cur, guylines=guylines))
            if towers:
                tower_objects.append(Tower(global_x=global_x_position, global_y=int_y_spacing_m,
                                       global_z=int_z_spacing_m, guylines=guylines, without_fence=without_fence))
    if towers:
        # Add in the end towers
        x_position = array_length_m / 2.0 + antenna_spacing_m / 2.0
        tower_objects.append(Tower(global_x=-x_position, global_y=int_y_spacing_m, guylines=guylines, without_fence=without_fence))
        tower_objects.append(Tower(global_x=x_position, global_y=int_y_spacing_m, guylines=guylines, without_fence=without_fence))
        ...
        for tower in range(0, num_antennas-1):
            ...
            tower_objects.append(Tower(global_x=x_position, global_y=int_y_spacing_m, start_height_m=12.0,
                                       guylines=guylines, without_fence=without_fence))
...
def max_coupling_card(*segment_pairs):
    ...
    integer (tag, segment) pairs, up to five of them, e.g. "CP 1 6 3 1". The
    previous version of this function returned "CP * * * *", which is not
    valid NEC2 and will fail to parse in strict readers such as xnec2c.

    If no segment pairs are supplied, no CP card is needed/emitted, since
    there is nothing meaningful to request coupling between.

    :param segment_pairs: Up to five (tag, segment) integer tuples to compute
                          coupling between
    :return: NEC compatible CP card string, or "" if no segment pairs given
    """
    if not segment_pairs:
        return ""
    fields = " ".join("{} {}".format(tag, segment) for tag, segment in segment_pairs)
    return "CP {}\r\n".format(fields)
...
#-----------------------------------------------------------------------------#
    ...
    freq_group.add_argument("-f", "--frequency", help="Single frequency to transmit on? MHz", default=None, type=str)
    freq_group.add_argument("--freq_range", help="Linear frequency sweep as 'start,stop,step' in MHz, e.g. '8,12,0.5'", default=None, type=str)
    freq_group.add_argument("--freq_list", help="Comma-separated list of arbitrary frequencies in MHz, e.g. '8.0,10.5,13.2'", default=None, type=str)

    parser.add_argument("-F", "--without_fence", help="Generate the array without the reflector fence", action="store_true")
...
#-----------------------------------------------------------------------------#
    ...
    if args.frequency is not None:
        ...
        # -f should only accept ONE frequency
        if "," in args.frequency:
            print(f"\n{RED}ERROR: -f / --frequency accepts only ONE frequency.")
            print("Did you mean --freq_list?")
            print("Example:")
            print(f"    --freq_list 9,10,11{RESET}")
            print()
            sys.exit(1)
        ...
        try:
            ...
            print(f"\n{RED}ERROR: '{args.frequency}' is not a valid frequency.")
            print("Expected a single frequency in MHz.")
            print("Example:")
            print(f"    -f 10.5{RESET}")
            print()
            sys.exit(1)
    ...
    if args.freq_list is not None:
        ...
        frequencies = args.freq_list.split(",")

        try:
            frequencies = [float(f.strip()) for f in frequencies]
        except ValueError:
            print(f"\n{RED}ERROR: Invalid --freq_list value: '{args.freq_list}'")
            print("Expected a comma-separated list of frequencies in MHz.")
            print("Example:")
            print(f"    --freq_list 9,10,11{RESET}")
            print()
            sys.exit(1)
        ...
        if len(frequencies) == 0:
            print(f"\n{RED}ERROR: --freq_list requires at least one frequency.")
            print("Example:")
            print(f"    --freq_list 9,10,11{RESET}")
            print()
            sys.exit(1)

        if (len(frequencies) == 3) and (frequencies[-1] < 2):
            print(f"\n{RED}ERROR: --freq_list last element less than 2.")
            print(f"{GREEN}If this was not an error, must change hardcoded value of 2 on line 1454 within this script{RED}")
            print("Did you mean --freq_range?")
            print("Example:")
            print("    --freq_range 9,11,0.5")
            print()
            sys.exit(1)

        # Store the cleaned list back if useful elsewhere
        args.freq_list = ",".join(str(f) for f in frequencies)
    ...
    if args.freq_range is not None:
        ...
        if len(parts) != 3:
            print(f"\n{RED}ERROR: Invalid --freq_range format.")
            print("Expected:")
            print("    --freq_range START,STOP,STEP")
            print()
            print("Example:")
            print("    --freq_range 9,11,0.5")
            print()
            print("For individual frequencies, did you mean --freq_list?")
            print(f"    --freq_list 9,10,11{RESET}")
            print()
            sys.exit(1)

        try:
            start, stop, step = [float(x.strip()) for x in parts]
        except ValueError:
            print(f"\n{RED}ERROR: Invalid --freq_range value: '{args.freq_range}'")
            print("Expected three numerical values:")
            print("    --freq_range START,STOP,STEP")
            print()
            print("Example:")
            print(f"    --freq_range 9,11,0.5{RESET}")
            print()
            sys.exit(1)

        if step <= 0:
            print(f"\n{RED}ERROR: Frequency range step must be greater than zero.")
            print(f"Received step: {step:g} MHz")
            print()
            print("Example:")
            print(f"    --freq_range 9,11,0.5{RESET}")
            print()
        ...
        if stop < start:
            print(f"\n{RED}ERROR: Frequency range STOP must be greater than or equal to START.")
            print(f"Received: start={start:g} MHz, stop={stop:g} MHz")
            print()
            print("Example:")
            print(f"    --freq_range 9,11,0.5{RESET}")
            print()
...
#-----------------------------------------------------------------------------#
### Terminal messages based on inputs and defaults ############################
#-----------------------------------------------------------------------------#
    if args.output_file is not None:
        print("{}Outputting data to:{} {}".format(GREEN, RESET, args.output_file))

    # Parse the input frequency(ies)
    freq_blocks = []
    if args.freq_range:
        start, stop, step = (float(x) for x in args.freq_range.split(','))
        num_steps = int(round((stop - start) / step)) + 1
        freq_blocks.append((start, num_steps, step))
    elif args.freq_list:
        for f in args.freq_list.split(','):
            freq_blocks.append((float(f), 1, 0.0))
    else:
        freq_blocks.append((float(args.frequency) if args.frequency is not None else 10.5, 1, 0.0))

    # Print the frequencies selected by the user
    if args.freq_list:
        frequencies = [float(f) for f in args.freq_list.split(',')]
        print("{}Selected frequencies:{} {}".format(GREEN, RESET,
            ", ".join(f"{f:g} MHz" for f in frequencies)
        ))
    elif args.freq_range:
        start, stop, step = map(float, args.freq_range.split(','))
        # Generate the actual frequencies in the range
        frequencies = []
        f = start
        while f <= stop + 1e-9:
            frequencies.append(f)
            f += step
        print("{}Selected frequency range:{}".format(GREEN, RESET))
        print(f"  {GREEN}Start:{RESET} {start:g} MHz")
        print(f"  {GREEN}Stop:{RESET}  {stop:g} MHz")
        print(f"  {GREEN}Step:{RESET} {step:g} MHz")
        print("  Frequencies: " +
              ", ".join(f"{f:g}" for f in frequencies) + " MHz")
    else:
        frequency = args.frequency if args.frequency is not None else 10.5
        print(f"{GREEN}Selected frequency:{RESET} {frequency:g} MHz")
    ...
    ## TODO: Check if is bugged from implimenting a list or range of frequencies
    reference_freq_mhz = freq_blocks[0][0]   # or create --phasing_frequency if you want it independent of the sweep
    if args.beam != BORESIGHT_BEAM:
        rel_phase = calculate_phase_from_beam(args.beam, reference_freq_mhz * 1e6, args.antenna_spacing)
    else:
        print(f"{GREEN}Using boresight{RESET}")
        rel_phase = 0
...
#-----------------------------------------------------------------------------#
    ...
    with open(args.output_file, 'w') as f:
        main_antenna_objects, main_tower_objects = create_main_array(args.antennas, args.antenna_spacing,
                                                                     ...
                                                                     without_fence=args.without_fence)

        int_antenna_objects, int_tower_objects = [], []
        ...
        # NEC2 requires comment (CM) cards to appear first in the file, terminated by
        # exactly one CE card, before any geometry (GW) cards. This used to be
        # written mid-file, after the EX cards, which produces an invalid deck for
        # strict NEC2 readers such as xnec2c (4nec2 tolerated it, but that's not
        # standard). Each antenna class's comment_string already ends with its own
        # "CE\r\n", so if we're combining a main-array and interferometer-array
        # comment block we have to strip every CE but the last one, or we'd emit
        # more than one CE card (equally invalid).
        comment_blocks = []
        if main_antenna_objects:
            comment_blocks.append(main_antenna_objects[0].repr_comment_string())
        if comment_blocks:
            stripped = [block.replace("CE\r\n", "", 1) for block in comment_blocks[:-1]]
            stripped.append(comment_blocks[-1])
        ...
        # max_coupling_card() takes explicit (tag, segment) pairs; called with no
        # arguments it returns "" (no CP card written) since this array doesn't use
        # coupling calculations. Previously this emitted the invalid "CP * * * *".
        ...
        for start_freq, num_steps, step_size in freq_blocks:
            f.write(frequency_card(num_freq_steps=num_steps, start_frequency=start_freq,
                                    freq_step_size=step_size))
