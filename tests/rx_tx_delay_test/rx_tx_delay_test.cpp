...
bool do_leading_edge_detection(
		cfloat* samples,
		size_t nsamp,
		size_t noise_est_window_len,
		float det_thresh_db,
		size_t* led_idx,
		float* led_val,
		float* noise_mu,
		...
		float* max_val)
{
	...
	sig /= ((float)noise_est_window_len);
	sig = sqrt(sig);
	...
	*noise_sig = sig;
	...
	for(size_t ii = noise_est_window_len; ii < nsamp; ii++)
	{
		...
		if(val_to_check > max_val_temp)
			max_val_temp = val_to_check;
	}
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
  ...
  size_t iterations;
	...
	desc.add_options()
        						("help", "help message")
        						("tx-addr", po::value<std::string>(&tx_addr)->default_value(""), "uhd transmit device address")
        						("rx-addr", po::value<std::string>(&rx_addr)->default_value(""), "uhd receive device address")
        						("args", po::value<std::string>(&args)->default_value(""), "uhd transmit and receive args")
        						...
        						("tx-gain", po::value<double>(&tx_gain)->default_value(0), "gain (dB) for the transmit RF chain")
        						("rx-gain", po::value<double>(&rx_gain)->default_value(0), "gain (dB) for the receive RF chain")
        						...
        						("tx-ref", po::value<std::string>(&tx_ref)->default_value("internal"), "clock reference (internal, external, mimo)")
        						("rx-ref", po::value<std::string>(&rx_ref)->default_value("internal"), "clock reference (internal, external, mimo)")
        						("otw", po::value<std::string>(&otw)->default_value("sc16"), "specify the over-the-wire sample mode")
        						("pps-sync", "if this switch is included then the usrp times will be synchronized using PPS")
        						("thresh",  po::value<double>(&thresh)->default_value(20), "detection threshold (dB) above noise sigma")
                    ("iterations", po::value<size_t>(&iterations), "How many iterations to run the LED code")
        						;
	po::variables_map vm;
	...
	if (vm.count("help")){
		...
		std::cout << "Example for WBX: " << std::endl
				...
				<< "--tx-rate=25000000 --rx-rate=25000000 --tx-freq=915000000 --rx-freq=915000000 "
				<< "--tx-ant=TX/RX --rx-ant=RX2 --tx-ref=gpsdo --rx-ref=mimo --pps-sync" << std::endl;
  ...
  if (vm.count("iterations")) {
	...
	std::cout << std::endl;
	std::cout << boost::format("Creating the dual usrp device with: %s...") % usrp_args << std::endl;
	uhd::usrp::multi_usrp::sptr dual_usrp = uhd::usrp::multi_usrp::make(usrp_args);
	...
	std::cout << boost::format("Using Device: %s") % dual_usrp->get_pp_string() << std::endl;

	//Lock mboard clocks
	tx_usrp->set_clock_source(tx_ref, 0);

	if(tx_ref != "internal")
		tx_usrp->set_time_source(tx_ref, 0);
	...
	if(rx_channel != 0)
	{
		...
		if(tx_ref != "internal")
			rx_usrp->set_time_source(rx_ref, rx_channel);
	}
	...
	std::cout << boost::format("Actual RX Rate: %f Msps...") % (rx_usrp->get_rx_rate(rx_channel)/1e6) << std::endl << std::endl;
	...
	if (vm.count("tx-ant"))
	{
		std::cout << boost::format("Setting TX Antenna: %s ...") % rx_ant << std::endl;
		rx_usrp->set_tx_antenna(tx_ant);
		std::cout << boost::format("Actual TX Antenna: %s ...") % (rx_usrp->get_tx_antenna()) << std::endl << std::endl;
	}
	...
	std::cout << boost::format("Actual RX Freq: %f MHz...") % (rx_usrp->get_rx_freq(rx_channel)/1e6) << std::endl << std::endl;

	//set the receive rf gain
	if (vm.count("rx-gain")){
		std::cout << boost::format("Setting RX Gain: %f dB...") % rx_gain << std::endl;
		rx_usrp->set_rx_gain(rx_gain, rx_channel);
		std::cout << boost::format("Actual RX Gain: %f dB...") % rx_usrp->get_rx_gain(rx_channel) << std::endl << std::endl;
	}
	...
	if (vm.count("rx-bw")){
		...
		std::cout << boost::format("Actual RX Bandwidth: %f MHz...") % (rx_usrp->get_rx_bandwidth(rx_channel)/1e6) << std::endl << std::endl;
	}
	...
	if (vm.count("rx-ant"))
	{
		...
		std::cout << boost::format("Actual RX Antenna: %s ...") % (rx_usrp->get_rx_antenna(rx_channel)) << std::endl << std::endl;
	}
	...
	if ((tx_ref == "mimo") and (std::find(tx_sensor_names.begin(), tx_sensor_names.end(), "mimo_locked") != tx_sensor_names.end())) {
		uhd::sensor_value_t mimo_locked = tx_usrp->get_mboard_sensor("mimo_locked",0);
		std::cout << boost::format("Checking TX: %s ...") % mimo_locked.to_pp_string() << std::endl;
		UHD_ASSERT_THROW(mimo_locked.to_bool());
	}
	if ((tx_ref == "gpsdo") and (std::find(tx_sensor_names.begin(), tx_sensor_names.end(), "gps_locked") != tx_sensor_names.end())) {
		uhd::sensor_value_t gpsdo_locked = tx_usrp->get_mboard_sensor("gps_locked",0);
		std::cout << boost::format("Checking TX: %s ...") % gpsdo_locked.to_pp_string() << std::endl;
		//UHD_ASSERT_THROW(gpsdo_locked.to_bool());
		if(!gpsdo_locked.to_bool())
		{
			std::cout << "Warning!  GPS is not locked, continuing anyway..." << std::endl;
		}
	}
	if ((tx_ref == "external" || tx_ref == "internal") and (std::find(tx_sensor_names.begin(), tx_sensor_names.end(), "ref_locked") != tx_sensor_names.end())) {
	...
	if ((rx_ref == "mimo") and (std::find(rx_sensor_names.begin(), rx_sensor_names.end(), "mimo_locked") != rx_sensor_names.end())) {
		uhd::sensor_value_t mimo_locked = rx_usrp->get_mboard_sensor("mimo_locked",rx_channel);
		std::cout << boost::format("Checking RX: %s ...") % mimo_locked.to_pp_string() << std::endl;
		UHD_ASSERT_THROW(mimo_locked.to_bool());
	}
	if ((rx_ref == "gpsdo") and (std::find(rx_sensor_names.begin(), rx_sensor_names.end(), "gps_locked") != rx_sensor_names.end())) {
		uhd::sensor_value_t gpsdo_locked = rx_usrp->get_mboard_sensor("gps_locked",rx_channel);
		std::cout << boost::format("Checking TX: %s ...") % gpsdo_locked.to_pp_string() << std::endl;
		//UHD_ASSERT_THROW(gpsdo_locked.to_bool());
		if(!gpsdo_locked.to_bool())
		{
	...
	if ((rx_ref == "external" || rx_ref == "internal") and (std::find(rx_sensor_names.begin(), rx_sensor_names.end(), "ref_locked") != rx_sensor_names.end())) {
		uhd::sensor_value_t ref_locked = rx_usrp->get_mboard_sensor("ref_locked",rx_channel);
	...
	std::cout << "Check Time Channel 0: " << boost::format("%.3f sec") % check_time0 << std::endl;
	std::cout << "Check Time Channel 1: " << boost::format("%.3f sec") % check_time1 << std::endl;
	std::cout << "Difference: " << boost::format("%.4f usec") % ((check_time1-check_time0)*1e6) << std::endl;

	double start_time = 7;

	boost::thread tx_thread(
			boost::bind(&do_tx, dual_usrp, otw, samps_per_buff, tx_rate, ampl,start_time));
		...
		double thresh)
{
	...
	try
	{
		...
		rx_buff_ptr = (std::complex<float>*)calloc(burst_length, sizeof(std::complex<float>));
		...
		double start_ahead = rx_padding*((double)samps_per_buff)/fs;

		std::cout << boost::format("Starting RX Ahead: %.4f msec") % (start_ahead*1e3) << std::endl;
		...
		while(!boost::this_thread::interruption_requested())
		{
			...
			while(!boost::this_thread::interruption_requested() && num_acc_samps < burst_length)
			{
				...
				packet_timeout = ((double)to_rx)/fs + 0.1;
				timeout = seconds_in_future + packet_timeout;;
        std::cout << "packettimeout: " << packet_timeout << " timeout: " << timeout << " Seconds in future: " << seconds_in_future <<std::endl;

				// get the data //
				num_samps_rxd = rx_stream->recv((rx_buff_ptr + num_acc_samps), to_rx, md, timeout);
				...
				{
					std::cout << boost::format("Timeout while streaming, received: %d out of samples %d (%.2f pct).")
					% num_acc_samps % burst_length % (((double)num_acc_samps)/((double)burst_length)*100) << std::endl;

					std::cout << "Tried To RX: " << to_rx << ", Loop Reps: " << reps << std::endl;
			...
			if(error_in_burst)
			{
				std::cout << "***** Error in RX Burst! Code: " << md.error_code << std::endl;
				if(overflow)
					std::cout << "\tRX Burst had Overflows: " << overflow_counts << std::endl;
			}

			//std::cout << "RX Burst Complete! " <<  num_acc_samps << " Samples Acquired." << std::endl;
			...
			do_leading_edge_detection(rx_buff_ptr, burst_length, 10*samps_per_buff, thresh, &led_idx, &led_val, &noise_mu, &noise_sig, &max_val);

			double tdiff = ((double)led_idx)/fs - start_ahead;

			std::cout << boost::format("Noise Mu: %.2f dB, Noise Sigma: %.2f dB, Max Value = %.2f dB")
			% (10*log10(noise_mu)) %  (10*log10(noise_sig)) %  (10*log10(max_val)) << std::endl;

			std::cout << boost::format("Detected at %d (of %d), LED Value = %.2f dB, TX/RX Timing Offset: %.4f usec, # Samples @ %.3f MHz: %d")
			% led_idx % burst_length %  (10*log10(led_val)) % (tdiff*1e6) % (fs/1000000) % ceil(tdiff *fs) << std::endl;

      count += 1;
      std::cout << "Count: " << count << std::endl;

			//double led_proc_time = usrp->get_time_now(rx_channel).get_real_secs() - usrp_time_now;

			// zero the receive buffer //
			memset(rx_buff_ptr, 0, sizeof(std::complex<float>)*burst_length);

			//std::cout << boost::format("Leading-Edge-Detection Processing Time: %.3f msec\n") % (led_proc_time*1e3) << std::flush;
		...
		double start_time)
{
	...
	try
	{
		...
		std::complex<float>* tx_buff_ptr = (std::complex<float>*)(send_buffer.data());
		...
		while(!boost::this_thread::interruption_requested())
		{
			...
			while(!boost::this_thread::interruption_requested() && num_acc_samps < burst_length)
			{
				...
				// set the timeout condition //
				packet_timeout = ((double)to_tx)/fs + 0.1;
				timeout = seconds_in_future + packet_timeout;

				// send the data //
				num_samps_txd = tx_stream->send((tx_buff_ptr + num_acc_samps), to_tx, md, timeout);
			...
			while (!boost::this_thread::interruption_requested() && usrp->get_device()->recv_async_msg(async_md))
			{
			...
			if(error_in_burst)
			{
				std::cout << "***** Error in TX Burst!  Last Error Code: " << last_error_code << std::endl;
			}
