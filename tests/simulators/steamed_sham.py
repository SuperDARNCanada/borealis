...
~~~~~~~~~~~~~~~~~~~~~~~~
This script mocks the usrp_driver module, creating artificial data.
All modules are implemented in Python, removing the need to install the C++ development
files, UHD, or CUDA in order to run. Note that without CUDA, the radar will run extremely
slowly.
...
def usage_msg():
    ...
    usage_message = """ steamed_sham.py [-h] experiment_module run_mode scheduling_mode_type

    Pass the module containing the experiment to steamed_sham as a required
...
BOREALISSCREENRC = """####THIS FILE IS GENERATED BY STEAMED_SHAM.PY####
...
sp.call("screen -ls | grep borealis | awk '{print $1}' | xargs -I{} screen -S {} -X quit", shell=True)
